...
from tensorflow.keras.layers import (Input, Embedding, LSTM, Bidirectional, GlobalAveragePooling1D, Dense, Dropout, SpatialDropout1D, BatchNormalization, Activation)
...
Models = [
    {
        ...
        "model_file": "SavedModels/lstm_aug_basic.keras",
        "sub_file": "ModelPredicts/submission_lstm_basic.csv"
    },
    {
        "name": "LSTM_NLTK",
        "train_file": "new_dataset/train_optimized_nltk.csv",
        "model_file": "SavedModels/lstm_aug_nltk.keras",
        "sub_file": "ModelPredicts/submission_lstm_nltk.csv"
    }
]

TEST_PATH = 'Dataset/test.csv'

# --- HYPERPARAMETERS (Tuned for Small Data LSTM) ---
MAX_WORDS = 8000        # Keep low to ignore rare noise
MAX_LEN = 250           # LSTMs are slower; 250 is plenty for sentiment
EMBEDDING_DIM = 32      # Small embedding to prevent memorization
BATCH_SIZE = 32         
EPOCHS = 30             
TEST_SIZE = 0.2         
...
def build_lstm_model(vocab_size, num_classes, input_length):
    inputs = Input(shape=(input_length,))
    
    embedding = Embedding(input_dim=vocab_size, output_dim=EMBEDDING_DIM)(inputs)
    # High Dropout is crucial for LSTMs on small data
    embedding = SpatialDropout1D(0.5)(embedding) 
    
    x = Bidirectional(LSTM(32, return_sequences=True, dropout=0.4, recurrent_dropout=0.4))(embedding)
    x = GlobalAveragePooling1D()(x)
    
    x = BatchNormalization()(x)
    x = Dense(32, kernel_regularizer=l2(0.02))(x)
    x = Activation('relu')(x)
    x = Dropout(0.5)(x)
...
if __name__ == "__main__":
    ...
    for exp in Models:
        ...
        model = build_lstm_model(MAX_WORDS, len(classes), MAX_LEN)
